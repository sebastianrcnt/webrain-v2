...
function EditArea() {
  ...
  if (
    t.settings["syntax_selection_allow"] &&
    t.settings["syntax_selection_allow"].length > 0
  )
    t.syntax_list = t.settings["syntax_selection_allow"]
      .replace(/ /g, "")
      .split(",");
...
EditArea.prototype.init = function () {
  ...
  if ((syntax_selec = _$("syntax_selection"))) {
    ...
    for (var i = 0; i < t.syntax_list.length; i++) {
      ...
      option.value = syntax;
      if (syntax == s["syntax"]) option.selected = "selected";
      dispSyntax = parent.editAreaLoader.syntax_display_name[syntax];
      option.innerHTML =
        typeof dispSyntax == "undefined"
          ? syntax.substring(0, 1).toUpperCase() + syntax.substring(1)
          : dispSyntax; //t.get_translation("syntax_" + syntax, "word");
  ...
  for (var i = 0; i < t.inlinePopup.length; i++) {
    if (t.isOpera) _$(t.inlinePopup[i]["popup_id"]).onkeypress = keyDown;
  ...
  for (var i = 0; i < children.length; i++) {
    if (t.isIE) children[i].unselectable = true;
    // IE
    else
  ...
  if (s["is_multi_files"] != false) t.open_file({ id: t.curr_file, text: "" });

  t.set_word_wrap(s["word_wrap"]);

  setTimeout(
    "editArea.focus();editArea.manage_size();editArea.execCommand('EA_load');",
    10
  );
  ...
  for (var i in t.plugins) {
    if (typeof t.plugins[i].onload == "function") t.plugins[i].onload();
  }
  if (s["fullscreen"] == true) t.toggle_full_screen(true);
  ...
  parent.editAreaLoader.add_event(window, "unload", function () {
    ...
    if (parent.editAreaLoader) {
      parent.editAreaLoader.remove_event(
        parent.window,
        "resize",
        editArea.update_size
      );
...
EditArea.prototype.update_size = function () {
  ...
  if (
    typeof editAreas != "undefined" &&
    editAreas[editArea.id] &&
    editAreas[editArea.id]["displayed"] == true
  ) {
    if (editArea.fullscreen["isFull"]) {
      pd.getElementById("frame_" + editArea.id).style.width =
        pd.getElementsByTagName("html")[0].clientWidth + "px";
      pd.getElementById("frame_" + editArea.id).style.height =
        pd.getElementsByTagName("html")[0].clientHeight + "px";
    }

    if (
      editArea.tab_browsing_area.style.display == "block" &&
      (!editArea.isIE || editArea.isIE >= 8)
    ) {
      editArea.tab_browsing_area.style.height = "0px";
      editArea.tab_browsing_area.style.height =
        editArea.result.offsetTop -
        editArea.tab_browsing_area.offsetTop -
        1 +
        "px";
    }
    ...
    for (i = 0; i < editArea.inlinePopup.length; i++) {
      ...
      if (popup.offsetTop > maxTop) popup.style.top = maxTop + "px";
      if (popup.offsetLeft > maxLeft) popup.style.left = maxLeft + "px";
    }
...
EditArea.prototype.manage_size = function (onlyOneTime) {
  if (!editAreas[this.id]) return false;
  ...
  if (editAreas[this.id]["displayed"] == true && this.textareaFocused) {
    ...
    if (this.settings["word_wrap"]) {
      ...
      if (this.isFirefox || this.isIE) newW -= 2;
      if (this.isSafari) newW -= 6;
      this.content_highlight.style.width = this.selection_field_text.style.width = this.selection_field.style.width = this.test_font_size.style.width =
        newW + "px";
    }
    ...
    if (this.last_selection["nb_line"] >= this.line_number) {
      var newLines = "",
        destDiv = _$("line_number"),
        start = this.line_number,
        end = this.last_selection["nb_line"] + 100;
  ...
  switch (cmd) {
    ...
    case "EA_unload":
      if (this.settings["EA_unload_callback"].length > 0)
        eval(
          "parent." +
            this.settings["EA_unload_callback"] +
            "('" +
            this.id +
            "');"
        );
    ...
    case "toggle_on":
      if (this.settings["EA_toggle_on_callback"].length > 0)
        eval(
          "parent." +
            this.settings["EA_toggle_on_callback"] +
            "('" +
            this.id +
            "');"
        );
    ...
    case "toggle_off":
      if (this.settings["EA_toggle_off_callback"].length > 0)
        eval(
          "parent." +
            this.settings["EA_toggle_off_callback"] +
            "('" +
            this.id +
            "');"
        );
    ...
    case "file_close":
      if (this.settings["EA_file_close_callback"].length > 0)
        return eval(
          "parent." + this.settings["EA_file_close_callback"] + "(param);"
        );
      break;

    default:
      if (typeof eval("editArea." + cmd) == "function") {
        if (this.settings["debug"]) eval("editArea." + cmd + "(param);");
        else
          try {
...
EditArea.prototype.get_translation = function (word, mode) {
  if (mode == "template")
    return parent.editAreaLoader.translate(
      word,
      this.settings["language"],
      mode
    );
  else
    return parent.editAreaLoader.get_word_translation(
      word,
      this.settings["language"]
    );
};
...
EditArea.prototype.add_plugin = function (plug_name, plug_obj) {
  for (var i = 0; i < this.settings["plugins"].length; i++) {
    if (this.settings["plugins"][i] == plug_name) {
      ...
      plug_obj.baseURL =
        parent.editAreaLoader.baseURL + "plugins/" + plug_name + "/";
      if (typeof plug_obj.init == "function") plug_obj.init();
    }
  }
};
...
EditArea.prototype.add_lang = function (language, values) {
  ...
  for (var i in values) parent.editAreaLoader.lang[language][i] = values[i];
};
