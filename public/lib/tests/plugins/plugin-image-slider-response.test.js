...
describe("image-slider-response", function () {
  ...
  test("loads correctly", function () {
    expect(typeof window.jsPsych.plugins["image-slider-response"]).not.toBe(
      "undefined"
    );
  });
  ...
  test("displays image stimulus", function (done) {
    var trial = {
      ...
      on_load: function () {
        ...
        utils.clickTarget(
          document.querySelector("#jspsych-image-slider-response-next")
        );
        done();
      },
    };
  ...
  test("should set min, max and step", function () {
    ...
    expect(
      jsPsych
        .getDisplayElement()
        .querySelector("#jspsych-image-slider-response-response").min
    ).toBe("2");
    expect(
      jsPsych
        .getDisplayElement()
        .querySelector("#jspsych-image-slider-response-response").max
    ).toBe("10");
    expect(
      jsPsych
        .getDisplayElement()
        .querySelector("#jspsych-image-slider-response-response").step
    ).toBe("2");
  ...
  test("should hide stimulus if stimulus_duration is set", function () {
    ...
    expect(
      jsPsych
        .getDisplayElement()
        .querySelector("#jspsych-image-slider-response-stimulus").style
        .visibility
    ).toMatch("");
