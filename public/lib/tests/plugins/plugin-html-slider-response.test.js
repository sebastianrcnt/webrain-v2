...
describe("html-slider-response", function () {
  ...
  test("loads correctly", function () {
    expect(typeof window.jsPsych.plugins["html-slider-response"]).not.toBe(
      "undefined"
    );
  });
  ...
  test("should set min, max and step", function () {
    ...
    expect(
      jsPsych
        .getDisplayElement()
        .querySelector("#jspsych-html-slider-response-response").min
    ).toBe("2");
    expect(
      jsPsych
        .getDisplayElement()
        .querySelector("#jspsych-html-slider-response-response").max
    ).toBe("10");
    expect(
      jsPsych
        .getDisplayElement()
        .querySelector("#jspsych-html-slider-response-response").step
    ).toBe("2");
  });
  ...
  test("should hide stimulus if stimulus_duration is set", function () {
    ...
    expect(
      jsPsych
        .getDisplayElement()
        .querySelector("#jspsych-html-slider-response-stimulus").style
        .visibility
    ).toMatch("");
  ...
  test("should end trial when button is clicked", function () {
    ...
    utils.clickTarget(
      document.querySelector("#jspsych-html-slider-response-next")
    );
