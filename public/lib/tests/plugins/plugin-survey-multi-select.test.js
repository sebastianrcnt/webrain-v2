...
describe("survey-multi-select plugin", function () {
  ...
  test("loads correctly", function () {
    expect(typeof window.jsPsych.plugins["survey-multi-select"]).not.toBe(
      "undefined"
    );
  });
  ...
  test("quoted values for options work", function () {
    ...
    expect(
      jsPsych
        .getDisplayElement()
        .querySelector("#jspsych-survey-multi-select-option-0-0 input").value
    ).toBe('Hello "boo"');
    expect(
      jsPsych
        .getDisplayElement()
        .querySelector("#jspsych-survey-multi-select-option-0-1 input").value
    ).toBe("yes, 'bar'");
  ...
  test("data are logged with the right question when randomize order is true", function () {
    ...
    document.querySelector(
      '#jspsych-survey-multi-select-0 input[value="a"]'
    ).checked = true;
    document.querySelector(
      '#jspsych-survey-multi-select-1 input[value="b"]'
    ).checked = true;
    document.querySelector(
      '#jspsych-survey-multi-select-2 input[value="c"]'
    ).checked = true;
    document.querySelector(
      '#jspsych-survey-multi-select-3 input[value="d"]'
    ).checked = true;
    document.querySelector(
      '#jspsych-survey-multi-select-4 input[value="e"]'
    ).checked = true;

    utils.clickTarget(
      document.querySelector("#jspsych-survey-multi-select-next")
    );
