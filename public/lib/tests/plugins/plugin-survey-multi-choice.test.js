...
describe("survey-multi-choice plugin", function () {
  ...
  test("loads correctly", function () {
    expect(typeof window.jsPsych.plugins["survey-multi-choice"]).not.toBe(
      "undefined"
    );
  });
  ...
  test("data are logged with the right question when randomize order is true", function () {
    ...
    document.querySelector(
      '#jspsych-survey-multi-choice-0 input[value="a"]'
    ).checked = true;
    document.querySelector(
      '#jspsych-survey-multi-choice-1 input[value="b"]'
    ).checked = true;
    document.querySelector(
      '#jspsych-survey-multi-choice-2 input[value="c"]'
    ).checked = true;
    document.querySelector(
      '#jspsych-survey-multi-choice-3 input[value="d"]'
    ).checked = true;
    document.querySelector(
      '#jspsych-survey-multi-choice-4 input[value="e"]'
    ).checked = true;

    utils.clickTarget(
      document.querySelector("#jspsych-survey-multi-choice-next")
    );
