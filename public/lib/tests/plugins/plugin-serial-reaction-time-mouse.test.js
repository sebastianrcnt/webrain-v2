...
describe("serial-reaction-time-mouse plugin", function () {
  ...
  test("loads correctly", function () {
    expect(
      typeof window.jsPsych.plugins["serial-reaction-time-mouse"]
    ).not.toBe("undefined");
  });
  ...
  test("default behavior", function () {
    ...
    expect(
      document.querySelector("#jspsych-serial-reaction-time-stimulus-cell-0-2")
        .style.backgroundColor
    ).toBe("");
    expect(
      document.querySelector("#jspsych-serial-reaction-time-stimulus-cell-0-3")
        .style.backgroundColor
    ).toBe("");

    utils.mouseDownMouseUpTarget(
      document.querySelector("#jspsych-serial-reaction-time-stimulus-cell-0-1")
    );
