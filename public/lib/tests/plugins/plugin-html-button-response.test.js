...
describe("html-button-response", function () {
  ...
  test("loads correctly", function () {
    expect(typeof window.jsPsych.plugins["html-button-response"]).not.toBe(
      "undefined"
    );
  });
  ...
  test("should hide stimulus if stimulus-duration is set", function () {
    ...
    expect(
      jsPsych
        .getDisplayElement()
        .querySelector("#jspsych-html-button-response-stimulus").style
        .visibility
    ).toMatch("");
  ...
  test("class should have responded when button is clicked", function () {
    ...
    expect(
      document.querySelector("#jspsych-html-button-response-stimulus").className
    ).toBe(" responded");
  });
});
