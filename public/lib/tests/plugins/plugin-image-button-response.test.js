...
describe("image-button-response", function () {
  ...
  test("loads correctly", function () {
    expect(typeof window.jsPsych.plugins["image-button-response"]).not.toBe(
      "undefined"
    );
  });
  ...
  test("display should clear after button click", function () {
    ...
    utils.clickTarget(
      document.querySelector("#jspsych-image-button-response-button-0")
    );
  ...
  test("should hide stimulus if stimulus-duration is set", function () {
    ...
    expect(
      jsPsych
        .getDisplayElement()
        .querySelector("#jspsych-image-button-response-stimulus").style
        .visibility
    ).toMatch("");
