...
describe("survey-text plugin", function () {
  ...
  test("default parameters work correctly", function () {
    ...
    expect(
      jsPsych.getDisplayElement().querySelectorAll("p.jspsych-survey-text")
        .length
    ).toBe(2);
    expect(
      jsPsych
        .getDisplayElement()
        .querySelector('input[name="#jspsych-survey-text-response-0"]').size
    ).toBe(40);
    expect(
      jsPsych
        .getDisplayElement()
        .querySelector('input[name="#jspsych-survey-text-response-1"]').size
    ).toBe(40);
  ...
  test.skip("required parameter works", function () {
    ...
    document.querySelector(
      'input[name="#jspsych-survey-text-response-0"]'
    ).value = "100";
