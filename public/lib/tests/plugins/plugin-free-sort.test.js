...
describe("free-sort plugin", function () {
  ...
  test("should be able to adjust the height and width of stimuli", function () {
    ...
    expect(jsPsych.getDisplayElement().innerHTML).toMatch(
      new RegExp(
        '<img src="img/happy_face_1.jpg" data-src="img/happy_face_1.jpg" class="jspsych-free-sort-draggable" draggable="false" style="position: absolute; cursor: move; width:200px; height:200px'
      )
    );
  });
  ...
  test("should display prompt", function () {
    ...
    expect(jsPsych.getDisplayElement().innerHTML).toMatch(
      new RegExp("<p>This is a prompt</p>")
    );
  });
  ...
  test('should display prompt at bottom if prompt_location is "below"', function () {
    ...
    expect(jsPsych.getDisplayElement().innerHTML).toMatch(
      new RegExp(
        '<p>This is a prompt</p><button id="jspsych-free-sort-done-btn"'
      )
    );
  });
