...
describe("image-keyboard-response", function () {
  ...
  test("loads correctly", function () {
    expect(typeof window.jsPsych.plugins["image-keyboard-response"]).not.toBe(
      "undefined"
    );
  });
  ...
  test("prompt should append html", function () {
    ...
    expect(jsPsych.getDisplayElement().innerHTML).toMatch(
      '<div id="foo">this is a prompt</div>'
    );
    utils.pressKey(70);
  });
  ...
  test("should hide stimulus if stimulus-duration is set", function () {
    ...
    expect(
      jsPsych
        .getDisplayElement()
        .querySelector("#jspsych-image-keyboard-response-stimulus").style
        .visibility
    ).toMatch("");
