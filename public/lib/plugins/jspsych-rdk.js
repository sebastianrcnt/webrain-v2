...
jsPsych.plugins["rdk"] = (function () {
  ...
  plugin.trial = function (display_element, trial) {
    ...
    trial.correct_choice = assignParameterValue(
      trial.correct_choice,
      undefined
    );
    trial.trial_duration = assignParameterValue(trial.trial_duration, 500);
    trial.response_ends_trial = assignParameterValue(
      trial.response_ends_trial,
      true
    );
    trial.number_of_apertures = assignParameterValue(
      trial.number_of_apertures,
      1
    );
    ...
    trial.coherent_direction = assignParameterValue(
      trial.coherent_direction,
      0
    );
    trial.coherence = assignParameterValue(trial.coherence, 0.5);
    trial.opposite_coherence = assignParameterValue(
      trial.opposite_coherence,
      0
    );
    ...
    trial.background_color = assignParameterValue(
    ...
    trial.aperture_center_x = assignParameterValue(
      trial.aperture_center_x,
      window.innerWidth / 2
    );
    trial.aperture_center_y = assignParameterValue(
      trial.aperture_center_y,
      window.innerHeight / 2
    );
    trial.fixation_cross = assignParameterValue(trial.fixation_cross, false);
    trial.fixation_cross_width = assignParameterValue(
      trial.fixation_cross_width,
      20
    );
    trial.fixation_cross_height = assignParameterValue(
      trial.fixation_cross_height,
      20
    );
    trial.fixation_cross_color = assignParameterValue(
      trial.fixation_cross_color,
      "black"
    );
    trial.fixation_cross_thickness = assignParameterValue(
      trial.fixation_cross_thickness,
      1
    );
    ...
    function end_trial() {
      ...
      if (frameRate.length > 0) {
        ...
        frameRate =
          frameRate.reduce((total, current) => total + current) /
          frameRate.length; //Sum up all the elements in the array
      } else {
      ...
      if (typeof trial.correct_choice !== "undefined") {
        ...
        if (trial.correct_choice.constructor === Array) {
          ...
          if (
            ...
            return trial.correct_choice.includes(
              String.fromCharCode(response.key)
    ...
    function setUpMultipleApertures() {
      ...
      for (
        currentApertureNumber = 0;
        currentApertureNumber < nApertures;
        currentApertureNumber++
      ) {
    ...
    function setParameter(originalVariable) {
      ...
      if (
        originalVariable.constructor === Array &&
    ...
    function initializeCurrentApertureParameters() {
      ...
      fixationCrossThickness =
        fixationCrossThicknessArray[currentApertureNumber];
      ...
      dotArray2d =
        dotArray3d.length !== 0 ? dotArray3d[currentApertureNumber] : undefined;
      ...
      for (var i = 0; i < nDots; i++) {
        ...
        if (RDK == 1) {
          ...
          //For opposite coherent dots
            ...
            i >= nCoherentDots &&
            i < nCoherentDots + nOppositeCoherentDots
          ) {
            dot = setvxvy(dot); // Set dot.vx and dot.vy
        ...
        if (RDK == 4) {
          ...
          dot = setvxvy(dot); // Set dot.vx and dot.vy
          dot.updateType =
            "constant direction or opposite direction or random position";
        } //End of RDK==4
        ...
        if (RDK == 5) {
          ...
          dot = setvxvy(dot); // Set dot.vx and dot.vy
          dot.updateType =
            "constant direction or opposite direction or random walk";
        } //End of RDK==5
        ...
        if (RDK == 6) {
          ...
          setvx2vy2(dot); // Set dot.vx2 and dot.vy2
          dot.updateType =
            "constant direction or opposite direction or random direction";
        } //End of RDK==6
    ...
    function updateDots() {
      ...
      if (currentSetArray[currentApertureNumber] == nSets - 1) {
        ...
        currentSetArray[currentApertureNumber] =
          currentSetArray[currentApertureNumber] + 1;
      }
      ...
      for (var i = 0; i < nDots; i++) {
        ...
        if (dot.updateType == "constant direction") {
          ...
          if (randomValue < coherence) {
            ...
            randomValue >= coherence &&
            randomValue < coherence + oppositeCoherence
          ) {
            dot = oppositeDirectionUpdate(dot);
          } else {
            dot = resetLocation(dot);
    ...
    function reinsertOnOppositeEdge(dot) {
      ...
      if (apertureType == 3 || apertureType == 4) {
        ...
        var weightOnHorizontalEdge =
          (horizontalAxis / (verticalAxis + horizontalAxis)) *
          weightInYDirection;
    ...
    function yValuePositive(x) {
      ...
      return (
        verticalAxis *
          Math.sqrt(1 - Math.pow(x, 2) / Math.pow(horizontalAxis, 2)) +
        apertureCenterY
    ...
    function xValuePositive(y) {
      ...
      return (
        horizontalAxis *
          Math.sqrt(1 - Math.pow(y, 2) / Math.pow(verticalAxis, 2)) +
        apertureCenterX
    ...
    function resetLocation(dot) {
      ...
      //For square and rectangle
        ...
        dot.y = randomNumberBetween(
          apertureCenterY - verticalAxis,
          apertureCenterY + verticalAxis
        ); //Between the top and bottom edges of the square / rectangle
      }
